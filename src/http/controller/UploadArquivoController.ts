...
export class UploadArquivoController {
    ...
    async upload(@UploadedFile() file, @Query('idProposta') idProposta: number,
        ...
        this.logger.log(`Uploading file, ${JSON.stringify(file.originalname)}`)
        this.logger.log(`idProposta, ${idProposta}`)
        this.logger.log(`idDocumento, ${idDocumento}`)
        ...
        await this.salvaDocumento.executar(idProposta, file.originalname);
    }
}